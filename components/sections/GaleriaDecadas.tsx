...
export default function GaleriaDecadas({ images }: { images?: string[] }) {
  ...
  return (
    <section className="bg-cream border-t border-[#6E5E53] py-12 md:py-16 overflow-hidden">
        ...
        <div className="flex gap-2 w-max animate-marquee hover:[animation-play-state:paused]">
          {loop.map((src, i) => (
            <img
              ...
              aria-hidden
              loading="lazy"
              className="h-[280px] md:h-[360px] w-[260px] md:w-[330px] object-cover flex-shrink-0 rounded-2xl"
            />
          ))}
        </div>
      </div>
