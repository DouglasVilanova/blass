...
export default function GaleriaEditor({ initialImages }: { initialImages: string[] }) {
  const { push } = useToast();
  ...
  async function save() {
    setSaving(true);
    const res = await saveSection("galeria", { images });
    setSaving(false);
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <section className="bg-white border border-brown/10 rounded-xl p-6">
        <h2 className="font-exo font-bold text-lg text-brown mb-1">Fotos da esteira</h2>
        <p className="text-xs text-brown/50 mb-4">
          As imagens são otimizadas (WebP) automaticamente antes de ir para o armazenamento.
          Rolam em loop contínuo no site.
        </p>
